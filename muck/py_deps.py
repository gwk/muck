...
from .pithy.fs import is_file, path_dir, path_join, path_split, norm_path
...
def py_dependencies(src_path:str, dir_names:Dict[str,Tuple[str,...]]) -> Iterable[str]:
  ...
  def extract_import(module_str:Optional[str], dir_names:Dict[str,Tuple[str,...]]) -> Optional[str]:
    ...
    for search_dir in search_dirs:
      ...
      try: cand_names = dir_names[cand_dir] # HACK: we are relying on this cache being populated. Should be a lazy structure.
  ...
  for node in ast.walk(tree):
    if isinstance(node, ast.Import):
      for alias in node.names:
        n = extract_import(alias.name, dir_names)
        if n: yield n
    elif isinstance(node, ast.ImportFrom):
      n = extract_import(node.module, dir_names)
      if n: yield n
